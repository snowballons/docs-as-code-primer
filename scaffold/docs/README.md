# Documentation root

| | |
|--|--|
| **Purpose** | Entry point for all documentation streams in this repository. |
| **Audience** | Everyone navigating docs |
| **Typical owner** | Tech Lead / Docs owner |
| **Phase** | All |
| **Stream** | Root |

## What belongs here

Entry point for all documentation streams in this repository.

## Do not put here

Application source code; secrets; one-off scratch notes that will never be reviewed.

## Next

- In-repo stub policy: [GOVERNANCE.md](GOVERNANCE.md)
- Upstream chapters / templates / examples: [KIT.md](KIT.md) (does not break after copy)
- This folder → upstream map: `appendix/phase-folder-map.md`
- Upstream template(s): `see KIT.md template index`
- Note: Streams: internal/, user/, shared/
