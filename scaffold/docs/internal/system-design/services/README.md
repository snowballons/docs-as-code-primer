# Service specifications

| | |
|--|--|
| **Purpose** | Per-service responsibility, interfaces, dependencies, error model. |
| **Audience** | Developers |
| **Typical owner** | Service owners |
| **Phase** | 3 |
| **Stream** | Internal |

## What belongs here

Per-service responsibility, interfaces, dependencies, error model.

## Do not put here

User-facing feature marketing pages.

## Next

- In-repo stub policy: [../../../GOVERNANCE.md](../../../GOVERNANCE.md)
- Upstream chapters / templates / examples: [../../../KIT.md](../../../KIT.md) (does not break after copy)
- This folder → upstream chapter: `primer/07-phase-3-detailed-design.md`
- Upstream template(s): `templates/module-spec.md`
